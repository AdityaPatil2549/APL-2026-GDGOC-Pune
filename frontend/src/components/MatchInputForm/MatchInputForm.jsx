...
export default function MatchInputForm({ onSubmit, locked }) {
  ...
  const updateBowler = (i, key, val) => setState(s => ({
    ...s,
    bowlers: s.bowlers.map((b, idx) => idx === i ? { ...b, [key]: val } : b)
  }))
  ...
  const validate = () => {
    ...
    if (state.wickets > 10) e.wickets = 'Max 10 wickets'
    if (state.innings === 2 && !state.target) e.target = 'Target required in 2nd innings'
    return e
  }
  ...
  const handlePasteUrl = async () => {
    ...
    // In real impl: call /api/scrape, get back match state
    // For demo: show loading then set default
  ...
  return (
    <form className={`match-form ${locked ? 'match-form--locked' : ''}`} onSubmit={handleSubmit} noValidate>
      ...
      <div className="match-form__url-bar">
        <input
          ...
          type="url"
          placeholder="Paste Cricbuzz match URL for auto-fill…"
          value={cricbuzzUrl}
        ...
        <button
          type="button"
          className="url-bar__btn"
          onClick={handlePasteUrl}
          disabled={locked || pasteLoading}
          id="paste-url-btn"
        >
          {pasteLoading ? '⏳' : '⚡ Fill'}
        </button>
      ...
      <FormGroup label="Innings">
        <PillToggle
          id="innings"
          options={[{ value: 1, label: '1st' }, { value: 2, label: '2nd' }]}
          value={state.innings}
      ...
      <FormGroup label="Dew Factor">
        <PillToggle
          ...
          value={state.dewFactor}
          onChange={v => set('dewFactor', v)}
        />
      </FormGroup>
      ...
      <div className="form-row form-row--score">
        ...
        {state.innings === 2 && (
          <FormGroup label="Target" htmlFor="target-input">
            <input id="target-input" type="number" className="form-input form-input--target" value={state.target || ''} min={0}
        ...
        {state.bowlers.map((b, i) => (
          <div key={i} className="bowler-table__row">
            ...
            <NumericStepper
              id={`bowler-overs-${i}`}
              value={b.oversUsed}
              onChange={v => updateBowler(i, 'oversUsed', v)}
              min={0} max={4} label={`Bowler ${i+1} overs`}
            />
            <span className={`bowler-table__left ${4 - b.oversUsed === 0 ? 'bowler-table__left--done' : ''}`}>
              {4 - b.oversUsed}
            </span>
            <button type="button" className="bowler-table__remove" onClick={() => removeBowler(i)} aria-label="Remove bowler">×</button>
          </div>
        ))}
        {state.bowlers.length < 5 && (
          <button type="button" className="bowler-table__add" onClick={addBowler} id="add-bowler-btn">
            + Add Bowler
          </button>
        )}
      </div>
      ...
      <div className="flags-grid">
        {[
          ...
          >
            {f.label}
            <span className="flag-btn__indicator">{state[f.key] ? 'YES' : 'NO'}</span>
          </button>
        ))}
      </div>
